/* global jQuery, FORTUNIO_STORAGE */

jQuery( document ).on( 'action.ready_fortunio', function() {
	...
	$document.on( 'action.resize_fortunio', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.fortunio_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		fortunio_set_cookie( 'fortunio_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	fortunio_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function fortunio_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						fortunio_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function fortunio_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function(response) {
						...
						fortunio_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						fortunio_import_tags_link( response );
						...
						if ( posts_container.length > 0 ) {
							...
							FORTUNIO_STORAGE['init_all_mediaelements'] = true;
							$document
	...
	$document.on( 'action.scroll_fortunio', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < fortunio_window_scroll_top() + fortunio_window_height() * 1.5 ) {
	...
	//-------------------------------------------------------------------------
	if ( ! FORTUNIO_STORAGE['select_container_disabled'] ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		fortunio_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					fortunio_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					fortunio_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		fortunio_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			fortunio_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function fortunio_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = fortunio_window_width() + ( fortunio_window_height() < $document.height() || fortunio_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = jQuery( 'table.cart' );   // table_cart;

				if ( w < FORTUNIO_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= FORTUNIO_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
